...
const init = function INIT() {
  ...
  const getRemotes = function GETREMOTES(errMessage) {
    ...
    if (!remotes || remotes.length === 0) {
      atom.notifications.addWarning(`Commit Live: ${errMessage}`, {
        dismissable: false,
      });
      return false;
    }
  ...
  const commands = {
    ...
    'greyatom-tree-view:connect': {
      ...
      command() {
        ...
        client.readConfig((e) => {
          if (e) {
            atom.notifications.addError('Commit Live: Could not read `.ftpconfig` file', {
              detail: e,
              dismissable: false,
            });
    ...
    'greyatom-tree-view:add-file': {
      ...
      command() {
        ...
        if (!(remotes[0] instanceof DirectoryView)) {
          atom.notifications.addError(`Commit Live: Cannot add a file to ${remotes[0].item.remote}`, {
            dismissable: false,
          });
          return;
        }
    ...
    'greyatom-tree-view:add-folder': {
      ...
      command() {
        ...
        if (remotes === false) return;
        if (!(remotes[0] instanceof DirectoryView)) {
          atom.notifications.addError(`Commit Live: Cannot add a folder to ${remotes[0].item.remote}`, {
            dismissable: false,
          });
          return;
        }
        ...
        dialog.on('path-changed', (e, newremote) => {
          client.rename(remotes[0].item.remote, newremote, (err) => {
            ...
            if (errMessage === 'file exists' || errMessage === 'File already exists') {
              atom.notifications.addError('Commit Live: File / Folder already exists', {
                dismissable: false,
              });
              return;
            }
    ...
    'greyatom-tree-view:download-selected-local': {
      ...
      command() {
        ...
        if (client.root.local === '') {
          atom.notifications.addError('Commit Live: You must define your local root folder in the projects .ftpconfig file.', {
            dismissable: false,
          });
          return;
        }
          ...
          var waitingForTransfers = setInterval(() => {
            if (attemptedTransfers == requestedTransfers) {
              ...
              if (successfulTransfers == requestedTransfers) {
                ...
                atom.notifications.addSuccess(`Commit Live: All transfers succeeded (${successfulTransfers} of ${requestedTransfers}).`);
              } else {
                        // :( some uploads failed
                atom.notifications.addError(`Commit Live: Some transfers failed<br />There were ${successfulTransfers} successful out of an expected ${requestedTransfers}.`);
              }
            }
          }, 200);
        }
          ...
          var waitingForTransfers = setInterval(() => {
            if (attemptedTransfers == locals.length) {
              ...
              if (successfulTransfers == locals.length) {
                        // great, all uploads worked
                atom.notifications.addSuccess(`Commit Live: All transfers succeeded (${successfulTransfers} of ${locals.length}).`);
              } else {
                        // :( some uploads failed
                atom.notifications.addError(`Commit Live: Some transfers failed<br />There were ${successfulTransfers} successful out of an expected ${locals.length}.`);
              }
            }
          }, 200);
        }
