...
const init = function INIT() {
  ...
  const getRemotes = function GETREMOTES(errMessage) {
    ...
    if (!remotes || remotes.length === 0) {
      atom.notifications.addWarning(`GreyAtom Tree View: ${errMessage}`, {
        dismissable: false,
      });
      return false;
    }
  ...
  const commands = {
    'greyatom-tree-view:create-ftp-config': {
      enabled: true,
      command() {
        createConfig({
          protocol: 'ftp',
    ...
    'greyatom-tree-view:create-sftp-config': {
      enabled: true,
      command() {
        createConfig({
          protocol: 'sftp',
    ...
    'greyatom-tree-view:create-ignore-file': {
      enabled: true,
      command() {
        if (!hasProject()) return;
    ...
    'greyatom-tree-view:toggle': {
      enabled: true,
      command() {
        remoteftp.treeView.toggle();
      },
    },
    'greyatom-tree-view:connect': {
      enabled: true,
      command() {
        if (!hasProject()) return;
        ...
        client.readConfig((e) => {
          if (e) {
            atom.notifications.addError('GreyAtom Tree View: Could not read `.ftpconfig` file', {
              detail: e,
              dismissable: false,
            });
    ...
    'greyatom-tree-view:disconnect': {
      enabled: true,
      command() {
        if (!hasProject()) return;

        client.disconnect();
        client.removeConfigFiles();
      },
    },
    'greyatom-tree-view:add-file': {
      enabled: true,
      command() {
        if (!hasProject()) return;
        ...
        if (!(remotes[0] instanceof DirectoryView)) {
          atom.notifications.addError(`GreyAtom Tree View: Cannot add a file to ${remotes[0].item.remote}`, {
            dismissable: false,
          });
          return;
        }
    ...
    'greyatom-tree-view:add-folder': {
      ...
      command() {
        ...
        if (remotes === false) return;
        if (!(remotes[0] instanceof DirectoryView)) {
          atom.notifications.addError(`GreyAtom Tree View: Cannot add a folder to ${remotes[0].item.remote}`, {
            dismissable: false,
          });
          return;
        }
    ...
    'greyatom-tree-view:refresh-selected': {
      enabled: true,
      command() {
        if (!hasProject()) return;
    ...
    'greyatom-tree-view:move-selected': {
      enabled: true,
      command() {
        if (!hasProject()) return;
        ...
        dialog.on('path-changed', (e, newremote) => {
          client.rename(remotes[0].item.remote, newremote, (err) => {
            ...
            if (errMessage === 'file exists' || errMessage === 'File already exists') {
              atom.notifications.addError('GreyAtom Tree View: File / Folder already exists', {
                dismissable: false,
              });
              return;
            }
    ...
    'greyatom-tree-view:delete-selected': {
      enabled: true,
      command() {
        if (!hasProject()) return;
    ...
    'greyatom-tree-view:download-selected': {
      enabled: true,
      command() {
        if (!hasProject()) return;
    ...
    'greyatom-tree-view:download-selected-local': {
      enabled: true,
      command() {
        if (!hasProject()) return;
        if (client.root.local === '') {
          atom.notifications.addError('GreyAtom Tree View: You must define your local root folder in the projects .ftpconfig file.', {
            dismissable: false,
          });
          return;
        }
        ...
        if (!client.isConnected()) {
          ...
          client.once('connected', () => {
            atom.commands.dispatch(atom.views.getView(atom.workspace), 'greyatom-tree-view:download-selected-local');
          });

          return;
        }

        if (atom.config.get('greyatom-tree-view.enableTransferNotifications')) {
          ...
          var waitingForTransfers = setInterval(() => {
            if (attemptedTransfers == requestedTransfers) {
              ...
              if (successfulTransfers == requestedTransfers) {
                ...
                atom.notifications.addSuccess(`GreyAtom Tree View: All transfers succeeded (${successfulTransfers} of ${requestedTransfers}).`);
              } else {
                        // :( some uploads failed
                atom.notifications.addError(`GreyAtom Tree View: Some transfers failed<br />There were ${successfulTransfers} successful out of an expected ${requestedTransfers}.`);
              }
            }
          }, 200);
        }
    ...
    'greyatom-tree-view:upload-selected': {
      enabled: true,
      command() {
        if (!hasProject()) return;
        ...
        if (!client.isConnected()) {
          ...
          atom.project.remoteftp.once('connected', () => {
            atom.commands.dispatch(atom.views.getView(atom.workspace), 'greyatom-tree-view:upload-selected');
          });
        ...
        locals.forEach((local) => {
          ...
          client.upload(local, (err, list) => {
            if (atom.config.get('greyatom-tree-view.enableTransferNotifications')) { attemptedTransfers++; }
            if (err) return;
            if (atom.config.get('greyatom-tree-view.enableTransferNotifications')) { successfulTransfers++; }
          ...
          var waitingForTransfers = setInterval(() => {
            if (attemptedTransfers == locals.length) {
              ...
              if (successfulTransfers == locals.length) {
                        // great, all uploads worked
                atom.notifications.addSuccess(`GreyAtom Tree View: All transfers succeeded (${successfulTransfers} of ${locals.length}).`);
              } else {
                        // :( some uploads failed
                atom.notifications.addError(`GreyAtom Tree View: Some transfers failed<br />There were ${successfulTransfers} successful out of an expected ${locals.length}.`);
              }
            }
          }, 200);
        }
    ...
    'greyatom-tree-view:upload-active': {
      enabled: true,
      command() {
        if (!hasProject()) return;
    ...
    // Remote -> Local
    'greyatom-tree-view:sync-with-remote': {
      enabled: true,
      command() {
        const remotes = remoteftp.treeView.getSelected();
    ...
    // Local -> Remote
    'greyatom-tree-view:sync-with-local': {
      enabled: true,
      command() {
        if (!hasProject()) return;
    ...
    'greyatom-tree-view:abort-current': {
      enabled: true,
      command() {
        if (!hasProject()) return;
        client.abort();
    ...
    'greyatom-tree-view:navigate-to': {
      enabled: true,
      command() {
        if (!hasProject()) return;
