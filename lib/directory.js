...
module.exports = Directory = (function (parent) {
  ...
  Directory.prototype.openPath = function (path, fileName, splitRight, beforeOpenCB) {
    let self = this,
      client = self.root.client;
    ...
    if (remainingPath.length > 0) {
      ...
      const fileMatcher = (file) => {
        if (file.name.toLowerCase() == fileName.toLowerCase()) {
          const fileLi = $('.greyatom-tree-view span[data-lower-path="' + path.toLowerCase() + '"]');
          if (fileLi.length) {
            fileLi.trigger('mousedown');
            // fileLi.trigger('dblclick');
            fileLi.view().item.open(splitRight, beforeOpenCB);
            return true;
          }
          return false;
        }
      ...
      let didOpened = false;
      if (remainingPathSplit.length > 0 && self.files.length > 0) {
        self.files.forEach((file) => {
          didOpened = fileMatcher(file);
        });
      }
      if (remainingPathSplit.length > 0 && !didOpened && self.folders.length > 0) {
        ...
        self.folders.forEach((folder) => {
          if (folder.remote === nextPath) {
            ...
            folder.open(false, () => {
              folder.openPath(path, fileName, splitRight, beforeOpenCB);
            });
          }
        });
      }
