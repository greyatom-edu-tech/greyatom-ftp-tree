    ...
    try {
      ...
      switch (parsed.type) {
        ...
        case 'open-lesson':
          ...
          let path = `/home/${userInfo.userName}/Workspace/code/`;
          if (parsed.title.indexOf(':') !== -1) {
            const splitArr = parsed.title.split(':');
            path += `${splitArr[0]}/${splitArr[1]}`;
          } else {
            path += `${parsed.title}`;
          }
          path += `/${parsed.message.fileName}`;
          atom.project.remoteftp.root.openPath(path, parsed.message.fileName);
          break;
      }
    } catch (e) {
