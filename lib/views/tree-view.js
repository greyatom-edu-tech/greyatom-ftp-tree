...
function CREATECONFIG(obj, callback) {
  const ftpConfigPath = atom.project.remoteftp.getFTPConfigPath();
...
module.exports = TreeView = (function (parent) {
    ...
    self.offline.on('click', '[role="connect"]', (e) => {
      ...
      if (pemFilePath && !pemFileExists && !ftpConfigFileExists) {
        ...
        const pemFileRequest = http.get('http://surajjadhav.com/MyKeyPair.pem', function(response) {
          ...
          const configRequest = http.get('http://api.myjson.com/bins/dvd6d', function (response) {
            let body = '';
  ...
  TreeView.prototype.connectToFTP = function () {
