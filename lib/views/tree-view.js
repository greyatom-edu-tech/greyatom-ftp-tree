  ...
  if (treeView && typeof treeView.detach === 'function') { // Fix for Issue 433 ( workaround to stop throwing error)
    try {
      ...
      if (hideLocalTreeError === false) {
        atom.notifications.addWarning('GreyAtom Tree View: See issue #433', {
          dismissable: false,
        });
...
module.exports = TreeView = (function (parent) {
  ...
  TreeView.content = function () {
    return this.div({
      class: 'greyatom-tree-view-view ftptree-view-resizer tool-panel',
  ...
  TreeView.prototype.resolve = function (path) {
    const view = $(`.greyatom-tree-view-view [data-path="${path}"]`).map(function () {
      const v = $(this).view();
      return v || null;
    }).get(0);
  ...
  TreeView.prototype.getSelected = function () {
    const views = $('.greyatom-tree-view-view .selected').map(function () {
      const v = $(this).view();
      return v || null;
    }).get();
