  ...
  */
  AssignmentView.prototype.initialize = function (track) {
    ...
    self.on('mousedown', function (e) {
      ...
      const currentTrack = localStorage.getItem('commit-live:current-track')
      if (currentTrack) {
        const parsedData = JSON.parse(currentTrack)
        if (parsedData.displayTitle == self.data.displayTitle) return;
      }
      view.open();
    });
  };
