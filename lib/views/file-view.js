...
module.exports = FileView = (function (parent) {
  ...
  FileView.prototype.initialize = function (file) {
    ...
    self.on('mousedown', function (e) {
      ...
      const $selected = $('.greyatom-tree-view .selected');
      ...
      if ((button === 0 || button === 2) && !(button === 2 && $selected.length > 1)) {
        if (!e[selectKey]) {
          ...
          $('.greyatom-tree-view .entries.list-tree').removeClass('multi-select');
        } else {
          $('.greyatom-tree-view .entries.list-tree').addClass('multi-select');
        }
