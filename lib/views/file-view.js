...
module.exports = FileView = (function (parent) {
  ...
  FileView.prototype.initialize = function (file) {
    ...
    self.name.attr('data-lower-path', self.item.remote.toLowerCase());
    ...
    self.on('mousedown', function (e) {
      ...
      const $selected = $('.greyatom-tree-view-view .selected');
      ...
      if ((button === 0 || button === 2) && !(button === 2 && $selected.length > 1)) {
        if (!e[selectKey]) {
          ...
          $('.greyatom-tree-view-view .entries.list-tree').removeClass('multi-select');
        } else {
          $('.greyatom-tree-view-view .entries.list-tree').addClass('multi-select');
        }
