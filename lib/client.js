  ...
  class Client extends EventEmitter {
    ...
    downloadPemFile(callback) {
      ...
      if (pemFileExists) {
        ...
        const apiForPem = userInfo.servers.pem_file_url;
        ...
        if (apiForPem.indexOf('https://') !== -1) {
          const pemFileRequest = https.get(apiForPem + 'student.pem', cb);
        } else {
          const pemFileRequest = http.get(apiForPem + 'student.pem', cb);
        }
      }
    }
    ...
    downloadFtpConfig(callback) {
      ...
      if (ftpConfigExists) {
        ...
        const ftpConfig = userInfoObj.servers.ftp_config;
        let parsed = ftpConfig;
    ...
    downloadIgnoreList(callback) {
      ...
      if (ftpIgnoreFileExists) {
        ...
        localStorage.setItem(ftpIgnoreConfigKey, ftpIgnoreConfig);
        callback(null);
      }
    }
