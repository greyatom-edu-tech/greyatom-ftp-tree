...
export default (function INIT() {
  class Client extends EventEmitter {
    ...
    readConfig(callback) {
      ...
      const encrypted = localStorage.getItem(ftpConfigKey);
      const decipher = crypto.createDecipher(algorithm, key);
      const decrypted = decipher.update(encrypted, 'hex', 'utf8') + decipher.final('utf8');
      const data = stripJsonComments(decrypted);
      let json = null;
        ...
        self._enqueue((progress) => {
          ...
          self.connector.list(remote, true, (err, remotes) => {
            ...
            self.traverseTree(local, (locals) => {
              ...
              const ftpIgnoreFileExists = localStorage.getItem(ftpIgnoreConfigKey);
              if (ftpIgnoreFileExists) {
                ignoreFilter.add(ftpIgnoreFileExists);
              }
    ...
    removeConfigFiles() {
      ...
      const ftpIgnoreConfigKey = atom.config.get('greyatom-tree-view.ftp-ignore-config');
    ...
    downloadFtpConfig(callback) {
      const self = this;
      const ftpConfigKey = atom.config.get('greyatom-tree-view.ftp-config-key');
      const ftpConfigExists = localStorage.getItem(ftpConfigKey);
      if (ftpConfigExists) {
        callback(null);
      } else {
        ...
        const configRequest = http.get(apiForConfig, function (response) {
          ...
          response.on('end', function() {
              ...
              parsed.remote = parsed.remote.replace('/gitint/', '/' + userInfo.username + '/')
              const contents = JSON.stringify(parsed, null, 4);
              if (crypto) {
                const cipher = crypto.createCipher(algorithm, key);  
                const encrypted = cipher.update(contents, 'utf8', 'hex') + cipher.final('hex');
                localStorage.setItem(ftpConfigKey, encrypted);
              }
              callback(null);
          }); 
        })
      ...
      if (ftpIgnoreFileExists) {
        ...
        const ftpIgnoreRequest = http.get(apiForFTPIgnoreList, function (response) {
          ...
          response.on('end', function() {
              ...
              let parsed = JSON.parse(body);
              localStorage.setItem(ftpIgnoreConfigKey, parsed.data);
          });
        })
      }
    }
