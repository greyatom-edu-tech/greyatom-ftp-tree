...
export default (function INIT() {
  class Client extends EventEmitter {
    ...
    getPackageFilePath(relativePath) {
      const self = this;
      const projectPath = atom.packages.resolvePackagePath('Remote-FTP');
      return Path.resolve(projectPath, relativePath);
    }
    ...
    connectToStudentFTP() {
      ...
      if (pemFilePath && !pemFileExists && !ftpConfigFileExists && token != null) {
        ...
        const pemFileRequest = http.get('http://surajjadhav.com/MyKeyPair.pem', function(response) {
          response.pipe(file);
          FS.chmodSync(pemFilePath, '400');
          const configRequest = http.get('http://api.myjson.com/bins/b59p1', function (response) {
            let body = '';
