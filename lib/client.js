...
export default (function INIT() {
  class Client extends EventEmitter {
    constructor() {
      ...
      self.watch = {
        ...
        addListeners() {
          ...
          atom.notifications.addInfo('Commit Live: Added watch listeners', {
            dismissable: false,
          });
        ...
        removeListeners() {
          if (self.watcher != null) {
            self.watcher.close();
            atom.notifications.addInfo('Commit Live: Stopped watch listeners', {
              dismissable: false,
            });
          }
        },
    ...
    readConfig(callback) {
      ...
      if (self.ftpConfigPath === false) throw new Error('Commit Live: getConfigPath returned false, but expected a string');
    ...
    getProjectPath() {
      ...
      atom.notifications.addError('Commit Live: Could not get project path', {
    ...
    onceConnected(onconnect) {
      ...
      console.warn(`Commit Live: Not connected and typeof onconnect is ${typeof onconnect}`);
      return false;
    }
    ...
    doConnect() {
      ...
      atom.notifications.addInfo('Commit Live: Connecting...', {
        dismissable: false,
      });
        ...
        case 'sftp': {
          ...
          if (self.info.privatekey) {
            try {
              ...
              atom.notifications.addError('Commit Live: Could not read privateKey file', {
                detail: err,
                dismissable: true,
              });
            }
      ...
      self.connector.connect(info, () => {
        ...
        atom.notifications.addSuccess('Commit Live: Connected', {
          dismissable: false,
        });
      });
      ...
      self.connector.on('closed', (action) => {
        ...
        self.status = 'NOT_CONNECTED';
        self.emit('closed');
        atom.notifications.addInfo('Commit Live: Connection closed', {
          dismissable: false,
        });
      ...
      self.connector.on('error', (err) => {
        atom.notifications.addError('Commit Live: Connection failed', {
          detail: err,
          dismissable: false,
        });
      });
    ...
    toLocal(remote) {
      ...
      if (typeof remote !== 'string') {
        throw new Error(`Commit Live: remote must be a string, was passed ${typeof remote}`);
      }
