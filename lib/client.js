...
export default (function INIT() {
  class Client extends EventEmitter {
    constructor() {
      ...
      self.watch = {
        ...
        addListeners() {
          ...
          atom.notifications.addInfo('GreyAtom Tree View: Added watch listeners', {
            dismissable: false,
          });
        ...
        removeListeners() {
          if (self.watcher != null) {
            self.watcher.close();
            atom.notifications.addInfo('GreyAtom Tree View: Stopped watch listeners', {
              dismissable: false,
            });
          }
        },
    ...
    readConfig(callback) {
      ...
      self.info = null;
      self.ftpConfigPath = self.getFTPConfigPath();

      if (self.ftpConfigPath === false) throw new Error('GreyAtom Tree View: getConfigPath returned false, but expected a string');
    ...
    getProjectPath() {
      ...
      atom.notifications.addError('GreyAtom Tree View: Could not get project path', {
    ...
    getFTPConfigPath() {
      return this.getPackageFilePath('./.ftpconfig');
    }

    getPackageFilePath(relativePath) {
      const projectPath = Path.join(homedir(), '/.atom/.greyatom-ide/');
    ...
    getFTPIgnoreFilePath() {
      return this.getPackageFilePath('./.ftpignore');
    }
    ...
    onceConnected(onconnect) {
        ...
        if (self.status === 'NOT_CONNECTED') {
          ...
          self.readConfig((err) => {
            if (err !== null) {
              ...
              // atom.notifications.addError("GreyAtom Tree View: " + err);
              return;
            }
            self.connect(true);
          });
      ...
      console.warn(`GreyAtom Tree View: Not connected and typeof onconnect is ${typeof onconnect}`);
      return false;
    }
    ...
    doConnect() {
      ...
      atom.notifications.addInfo('GreyAtom Tree View: Connecting...', {
        dismissable: false,
      });
        ...
        case 'sftp': {
          ...
          if (self.info.privatekey) {
            try {
              ...
              atom.notifications.addError('GreyAtom Tree View: Could not read privateKey file', {
                detail: err,
                dismissable: true,
              });
            }
      ...
      self.connector.connect(info, () => {
        ...
        atom.notifications.addSuccess('GreyAtom Tree View: Connected', {
          dismissable: false,
        });
      });
      ...
      self.connector.on('closed', (action) => {
        ...
        self.status = 'NOT_CONNECTED';
        self.emit('closed');
        atom.notifications.addInfo('GreyAtom Tree View: Connection closed', {
          dismissable: false,
        });
      ...
      self.connector.on('error', (err) => {
        atom.notifications.addError('GreyAtom Tree View: Connection failed', {
          detail: err,
          dismissable: false,
        });
      });
    ...
    toLocal(remote) {
      ...
      if (typeof remote !== 'string') {
        throw new Error(`GreyAtom Tree View: remote must be a string, was passed ${typeof remote}`);
      }
    ...
    removeConfigFiles() {
      ...
      this.deleteFile(this.getFTPConfigPath());
      this.deleteFile(this.getFTPIgnoreFilePath());
    }
    ...
    downloadPemFile(callback) {
      ...
      if (pemFileExists) {
        ...
        const apiForPem = atom.config.get('greyatom-tree-view.api-for-pem');
        const pemFileRequest = http.get(apiForPem, function(response) {
          response.pipe(file);
          FS.chmodSync(pemFilePath, '400');
          callback(null);
        });
      }
    }

    downloadFtpConfig(callback) {
      const self = this;
      const ftpConfigFilePath = self.getFTPConfigPath();
      const ftpConfigFileExists = FS.existsSync(ftpConfigFilePath);
      if (ftpConfigFileExists) {
        callback(null);
      } else {
        const apiForConfig = atom.config.get('greyatom-tree-view.api-for-ftpconfig');
        const configRequest = http.get(apiForConfig, function (response) {
          let body = '';
          ...
          response.on('end', function() {
              ...
              parsed.privatekey = self.getPemFilePath();
              const contents = JSON.stringify(parsed, null, 4);
              FS.writeFile(ftpConfigFilePath, contents, (err) => {
    ...
    downloadIgnoreList(callback) {
      const self = this;
      const ftpIgnorePath = self.getFTPIgnoreFilePath();
      const ftpIgnoreFileExists = FS.existsSync(ftpIgnorePath);
      if (ftpIgnoreFileExists) {
        callback(null);
      } else {
        const apiForFTPIgnoreList = atom.config.get('greyatom-tree-view.api-for-ftpignore');
        const ftpIgnoreRequest = http.get(apiForFTPIgnoreList, function (response) {
          let body = '';
          response.on('data', function(d) {
              body += d;
          });
          response.on('end', function() {
              // Data reception is done, do whatever with it!
              let parsed = JSON.parse(body);
              FS.writeFile(ftpIgnorePath, parsed.data, (err) => {
                if (!err) callback(null);
              });
          }); 
        })
    ...
    connectToStudentFTP() {
      ...
      series([
          (callback) => {
            self.downloadPemFile.call(self, callback)
          },
          (callback) => {
            self.downloadFtpConfig.call(self, callback)
          },
          (callback) => {
            self.downloadIgnoreList.call(self, callback)
          }
        ],
        function (err) {
          if (!err) self.connectToFTP();
        });
    }
