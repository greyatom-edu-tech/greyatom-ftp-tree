...
export default (function INIT() {
  class Client extends EventEmitter {
    ...
    getPackageFilePath(relativePath) {
      const self = this;
      const projectPath = atom.packages.resolvePackagePath('greyatom-tree-view');
      return Path.resolve(projectPath, relativePath);
    }
    ...
    getLocalProjectPath() {
      const projectPath = atom.packages.resolvePackagePath('greyatom-tree-view');
      return Path.resolve(projectPath, 'project');
    }
    ...
    toRemote(local) {
      const self = this;
      let strToMatch = 'project/';
      let regForReplace = /^.*project/;
      if (process.platform == 'win32') {
        strToMatch = 'project\\';
        regForReplace = /^.*project\\/;
      }
    ...
    createConfig(obj, callback) {
      const ftpConfigPath = this.getFTPConfigPath();
    ...
    connectToStudentFTP() {
      const self = this;
      if (self.isConnected()) return;
      const pemFilePath = self.getPemFilePath();
      const ftpConfigFilePath = self.getFTPConfigPath();
      const pemFileExists = FS.existsSync(pemFilePath);
      const ftpConfigFileExists = FS.existsSync(ftpConfigFilePath);
      const tokenKey = atom.config.get('greyatom-tree-view.student-token');
      ...
      if (pemFilePath && !pemFileExists && !ftpConfigFileExists) {
        ...
        const pemFileRequest = http.get(apiForPem, function(response) {
          ...
          const configRequest = http.get(apiForConfig, function (response) {
            ...
            response.on('end', function() {
                ...
                parsed.privatekey = pemFilePath;
                self.createConfig(parsed, () => {
                  self.connectToFTP();
                });
            }); 
          })
        });
      } else if (pemFileExists && ftpConfigFileExists) {
        self.connectToFTP();
      }
    }
