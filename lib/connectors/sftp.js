...
module.exports = (function () {
  ...
  ConnectorSFTP.prototype.connect = function (info, completed, connectingPopup) {
    const self = this;
    self.info = info;
    ...
    try {
      ...
      // });
      if (connectingPopup) {
        connectingPopup.dismiss();
      }
      setTimeout(function () {
        if (atom.project.remoteftp) {
          atom.project.remoteftp.connectToStudentFTP('RECONNECTING');
        }
      }, 0);
    }
  ...
  if (!obj.hasOwnProperty('err') && (err.message == 'No such file' || err.message === 'NO_SUCH_FILE')) {
    self.mkdir(Path.dirname(remote).replace(/\\/g, '/'), true, (err) => {
      if (err) {
        ...
        atom.notifications.addError(`Commit Live: Upload Error ${error}`, {
          dismissable: false,
        });
        return err;
      }
